...
main (int argc, char *argv[])
{
  ...
  UdpEchoClientHelper udpClient (groupAddress , 8000);
  UdpEchoServerHelper udpServer (8000);
  ...
  ApplicationContainer serverApps = udpServer.Install (ueResponders);
  serverApps.Start(Seconds (1.0));
  serverApps.Stop(Seconds (1000.0));
  clientApps.Get(0)->SetStartTime (Seconds (3.0));
  clientApps.Get(1)->SetStartTime (Seconds (6.0));
