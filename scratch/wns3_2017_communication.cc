...
void ReceptionTraceCallBack(Ptr<OutputStreamWrapper> stream,uint16_t m_rnti, uint16_t id, double rsrp, double rsrq, bool serving ){

	NS_LOG_INFO(" this is node "<<m_rnti <<" getting values of "<<rsrp << " and rsrq "<< rsrq);
...
{
	LogComponentEnable("NistUdpEchoClientApplication", LOG_ALL);
	LogComponentEnable("NistPacketSink", LOG_ALL);
	LogComponentEnable("d2d_communication_mode_2", LOG_ALL);
	//LogComponentEnable("NistLteUePhy", LOG_ALL);
  ...
  // Set pathloss model
  Config::SetDefault ("ns3::Nist3gppPropagationLossModel::CacheLoss", BooleanValue (true));
  ...
  *stream->GetStream () << "time\ttx/rx\tNID\tIMSI\tUEtype\tsize\tIP[src]\tIP[dst]" << std::endl;
  ...
  for (uint16_t ac = 0; ac < clientRespondersApps.GetN (); ac++)
    {
      oss << "t\t" << activeTxUes.Get(ac)->GetNode()->GetId () << "\t" << activeTxUes.Get(ac)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\tresp";
  ...
  for (uint16_t ac = 0; ac < clientRespondersSrvApps.GetN (); ac++)
    {
      ...
      oss << "r\t" << ueResponders.Get (ac)->GetId () << "\t" << ueResponders.Get (ac)->GetDevice (0)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\tresp";
  ...
  Ptr<NetDevice> netDevice = ueResponders.Get(0)->GetDevice(0);
  Ptr<NistLteUeNetDevice> NistnetDevice = DynamicCast<NistLteUeNetDevice>(netDevice);
  std::ostringstream oss2;
  	oss2.str("");
AsciiTraceHelper ascii11;
	Ptr<OutputStreamWrapper> stream1 = ascii11.CreateFileStream(
			"reception_mesures_phy.tr");
	*stream1->GetStream() << "Time\tIMSI\tCellId\tRNTI\tProSeAppCode"
			<< std::endl;


  NistnetDevice->TraceConnect("ReportNistUeMeasurements", oss.str(),
			MakeBoundCallback(&ReceptionTraceCallBack, stream1));
  //Ptr<NistLteUePhy> phy = netDevice->GetObject<NistLteUePhy>() ;
  NS_LOG_INFO ("Starting simulation..."<<NistnetDevice->GetAddress());
