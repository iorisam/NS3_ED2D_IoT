...
NS_LOG_COMPONENT_DEFINE ("NistWns32017Discovery");
...
{  
  ...
  /*
  Config::SetDefault ("ns3::NistLteEnbNetDevice::DlEarfcn", StringValue ("5330"));
  Config::SetDefault ("ns3::NistLteUeNetDevice::DlEarfcn", StringValue ("5330"));
  ...
  */
  ...
  Config::SetDefault ("ns3::NistLteUePhy::TxPower", DoubleValue (23.0));
  //Config::SetDefault ("ns3::NistLteUePowerControl::Pcmax", DoubleValue (24.0));
  // Set the eNBs power in dBm
  Config::SetDefault ("ns3::NistLteEnbPhy::TxPower", DoubleValue (46.0));
  ...
  Config::SetDefault ("ns3::NistLteEnbRrc::SrsPeriodicity", UintegerValue (80));
  ...
  Config::SetDefault ("ns3::NistLteSpectrumPhy::ErrorModelHarqD2dDiscoveryEnabled", BooleanValue (useRecovery));
  ...
  *st->GetStream () << "id\tx\ty" << std::endl;
  ...
  preconfiguration.preconfigDisc.pools[0].txParameters.txProbability = NistLteRrcSap::TxProbabilityFromInt (txProb);
  ...
  {
    announceApps[ueDevs.Get(i-1)].push_back (std::to_string(i));
    for (uint32_t j=1; j<= nbUes; ++j)
    {
      if (i != j)
      {
        monitorApps[ueDevs.Get(i-1)].push_back (std::to_string(j));
      }
    }
  }
