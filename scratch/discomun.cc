...
void SlBearerActivation (Ptr<NistLteHelper> helper, NetDeviceContainer ues, Ptr<NistSlTft> tft)
{
...
{
	LogComponentEnable("NistUdpEchoClientApplication", LOG_LEVEL_LOGIC);
	LogComponentEnable("NistPacketSink", LOG_LEVEL_LOGIC);
...
AsciiTraceHelper asc;
  ...
  *st->GetStream () << "id\tx\ty" << std::endl;
      ...
      for (uint32_t i = 0; i < ueAllNodes.GetN (); ++i)
        {
          ...
          m_outFile << " UE\t" << ueNetDevice->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\t" << ueAllNodes.Get (i)->GetObject<MobilityModel> ()->GetPosition ().x << "\t" << ueAllNodes.Get (i)->GetObject<MobilityModel> ()->GetPosition ().y << std::endl;
        }
      m_outFile.close ();
    }
  ...
  // Create groups
  double ulEarfcn = enbDevs.Get (0)->GetObject<NistLteEnbNetDevice> ()->GetUlEarfcn ();
  double ulBandwidth = enbDevs.Get (0)->GetObject<NistLteEnbNetDevice> ()->GetUlBandwidth ();
  std::vector < NetDeviceContainer > createdgroups;
  if (nbreceivers > 0)
    {
      //groupcast
      createdgroups = proseHelper->AssociateForGroupcast (ueTxPower, ulEarfcn, ulBandwidth, ueRespondersDevs, -112, nbgroups, nbreceivers, NistLteProseHelper::SRSRP_EVAL);
    }
  else
    {
      //broadcast
      createdgroups = proseHelper->AssociateForBroadcastWithTxEnabledToReceive (ueTxPower, ulEarfcn, ulBandwidth, ueRespondersDevs, -112, nbgroups, NistLteProseHelper::SRSRP_EVAL);
    }
  ...
  AsciiTraceHelper ascii5;
  Ptr<OutputStreamWrapper> stream5 = ascii5.CreateFileStream ("prose_connections.txt");
  proseHelper->PrintGroups (createdgroups, stream5);
  ...
  stream5 = ascii5.CreateFileStream ("ue_pck.tr");
  //Trace file table header
  *stream5->GetStream () << "time\ttx/rx\tNID\tIMSI\tUEtype\tsize\tIP[src]\tIP[dst]" << std::endl;
  ...
  for (gIt = createdgroups.begin() ; gIt != createdgroups.end() ; gIt++)
    {
      ...
      Ptr<NistSlTft> tft = Create<NistSlTft> (NistSlTft::TRANSMIT, clientRespondersAddress, groupL2Address);
      proseHelper->ActivateSidelinkBearer (Seconds(1.0), txUe, tft);
      tft = Create<NistSlTft> (NistSlTft::RECEIVE, clientRespondersAddress, groupL2Address);
      ...
      if (onoff)
        {
          ...
          NistOnOffHelper clientOnOffHelper ("ns3::UdpSocketFactory",
        ...
        {
          // UDP application
          NistUdpEchoClientHelper echoClientHelper (clientRespondersAddress, application_port);
  ...
  NistPacketSinkHelper clientPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), application_port));
  ...
  for (uint16_t ac = 0; ac < clientRespondersApps.GetN (); ac++)
    {
      oss5 << "t\t" << activeTxUes.Get(ac)->GetNode()->GetId () << "\t" << activeTxUes.Get(ac)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\tresp";
      ...
      clientRespondersApps.Get (ac)->TraceConnect("TxWithAddresses", oss5.str (), MakeBoundCallback(&PacketSrcDstAddrsTrace, stream5, local_addrs));
      oss5.str("");
    }
  ...
  for (uint16_t ac = 0; ac < clientRespondersSrvApps.GetN (); ac++)
    {
      ...
      oss5 << "r\t" << ueResponders.Get (ac)->GetId () << "\t" << ueResponders.Get (ac)->GetDevice (0)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\tresp";
      clientRespondersSrvApps.Get (ac)->TraceConnect("RxWithAddresses", oss5.str (), MakeBoundCallback(&PacketSrcDstAddrsTrace, stream5, local_addrs));
      oss5.str("");
    }
...
std::map<Ptr<NetDevice>, std::list<std::string> > announceApps;
  ...
  for (uint32_t i=1; i<= ueAllNodes.GetN (); ++i)
  {
    announceApps[ueRespondersDevs.Get(i-1)].push_back (std::to_string(i));
    for (uint32_t j=1; j<= ueAllNodes.GetN (); ++j)
    {
      if (i != j)
      {
  ...
  *stream2->GetStream () << "Time\tIMSI\tRNTI\tProSeAppCode" << std::endl;
  ...
  {
    Ptr<NistLteUeRrc> ueRrc = DynamicCast<NistLteUeRrc>( ueRespondersDevs.Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc () );    
    ueRrc->TraceConnectWithoutContext ("DiscoveryMonitoring", MakeBoundCallback (&DiscoveryMonitoringTrace, stream));
    oss << ueRespondersDevs.Get (i)->GetObject<NistLteUeNetDevice> ()->GetImsi ();
    Ptr<NistLteUePhy> uePhy = DynamicCast<NistLteUePhy>( ueRespondersDevs.Get (i)->GetObject<NistLteUeNetDevice> ()->GetPhy () );
  //  uePhy->TraceConnect ("DiscoveryAnnouncement", oss.str (), MakeBoundCallback (&DiscoveryAnnouncementPhyTrace, stream1));
    Ptr<NistLteUeMac> ueMac = DynamicCast<NistLteUeMac>( ueRespondersDevs.Get (i)->GetObject<NistLteUeNetDevice> ()->GetMac () );
