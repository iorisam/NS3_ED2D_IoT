...
#ifndef LTE_ECHO_APP_H
#define LTE_ECHO_APP_H
 ...
 */
class Lte_Echo_Client : public Application
{
public:
  /**
  ...
  void (*count_cover)(std::list<int> newlist) = 0;

  Lte_Echo_Client ();

  virtual ~Lte_Echo_Client ();
...
private:
  ...
  int CalculateMaxSf(std::map<Address,neighbor> dominating_nodes,int m_coverage_degree);
  bool LoadBalance();
  ...
  double CalculateBestTxPower(std::map<Address, neighbor> covered, std::map<uint16_t,double> CapturedRssi);
...
public : std::list<Ptr<Packet>> packets_list;
public : std::list<Ptr<Packet>> neighbors_packets_list;
public : EnergyModuleLte *my_energy;
...
int cdsStates[100];
int number_of_types_of_services;
std::map<uint16_t,double> CapturedRssi;
