#ifndef NEED_PROSE_APP_H
#define NEED_PROSE_APP_H
...
class Need_ProSe_App : public Application
{
public:

	 static TypeId GetTypeId (void);
	 Need_ProSe_App ();
	 virtual ~Need_ProSe_App ();
	 /**
	 ...
	 private:
	   ...
	   void SetProSeDiscoveryMessage(std::string message);
	   void ResetProSeDiscovery();
	   void SetupProSeDiscoveryCallback();
	 ...
	 // cluster head nodes
	  ...
	  //my calculated mainhead broadcast time
	  double m_rand_time;

	  //vice ch received broadcast time
	  double v_rand_time;

	  std::list<std::string>  announceApp;
	  std::list<std::string>  monitorApps;

	  Ptr<NistLteHelper> lte_helper;

	  std::vector<EnergyModuleLte *> energy_sources;
	  void DiscoveryMonitoringCallback ( uint16_t sender, std::string proSeAppCode);
	  void handleDiscoveryMessage (uint16_t sender, std::string DiscMsg);
	  double ParseRandTime(std::string data);
