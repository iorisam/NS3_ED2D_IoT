...
void EnergyModuleLte::Only_uplink_tx(double idleTime)
{
	...
	double supplyVoltage=4.0;
	double energyDecreasedIdle=0*supplyVoltage*idleTime/1000000000;
...
//		std::cout<<"Transmit chain is consuming power"<< (energyDecreasedTx+energyDecreasedIdle)
//				<<" joules and power remaining is "<< LIES.m_remaining<< std::endl;

	}

	if(60.0-LIES.m_remaining >= 5){
		LIES.HandleEnergyDrainedEvent();
	}
...
//		std::cout<<"Receive chain is consuming power"<< (energyDecreasedRx+energyDecreasedIdle)
//		<<" joules and power remaining is "<<std::endl;
	}
...
void EnergyModuleLte::both_donwlink_and_uplink(double idleTime)
{
	...
	only_tx = MakeCallback(&LiIonEnergySource::DecreaseRemainingEnergy, &LIES);
	double supplyVoltage=4.0;
	double energyDecreasedIdle=0*supplyVoltage*0.001;
	only_tx(energyDecreasedIdle);
