    ...
    m_lastUpdateTime (Seconds (0.0))
{
  ...
  m_initialEnergyJ=60.0;
  m_remainingEnergyJ=m_initialEnergyJ;
  m_remaining = m_initialEnergyJ;
}
...
LiIonEnergySource::DecreaseRemainingEnergy (double energyJ)
{
  ...
  m_remaining = m_remainingEnergyJ;
  //std::cout<<Simulator::Now().GetMilliSeconds()<<" "<< "ueid " << this << " Remaining energy in joules " << (double)m_remainingEnergyJ << std::endl;
  // check if remaining energy is 0
  if (m_supplyVoltageV <= m_minVoltTh)
    {
      //std::cout<<"Inside Energy Depletion\n";
...
LiIonEnergySource::UpdateEnergySource (void)
{
  ...
  //m_lowBatteryTh
  if (m_remaining <=  m_lowBatteryTh *  m_initialEnergyJ)
    {
      HandleEnergyDrainedEvent ();
      return; // stop periodic update
    }

  if (m_initialEnergyJ - m_remaining >=  55  )
      {
...
LiIonEnergySource::HandleEnergyDrainedEvent (void)
{
  ...
  if(!LteEnergyDepletionCallback.IsNull())
  LteEnergyDepletionCallback( (double)GetNode ()->GetId ());
}
