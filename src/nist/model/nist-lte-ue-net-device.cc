...
{
  NS_LOG_FUNCTION (this);
  std::cout<<" this is my m_phy "<<m_phy->GetDevice();
  return m_phy;
}
...
NistLteUeNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  if (protocolNumber != Ipv4L3Protocol::PROT_NUMBER)
    {
      ...
      std::cout<<"unsupported protocol " << protocolNumber << ", only IPv4 is supported"<<std::endl;
      return true;
    }
  ...
  Ptr<Packet> packet2 = packet->Copy ();
               //m_ltePhyTxEndCallback (packet);
                SeqTsHeader seqTs;
     		   packet2->RemoveHeader (seqTs);
     		   uint32_t currentSequenceNumber = seqTs.GetSeq ();
     		   //ProSeTag tagCopy;
     		  //packet2->PeekPacketTag (tagCopy);
     		 //Ipv4Header ipHeader;
     		//packet2->PeekHeader(ipHeader);
     		/* if(currentSequenceNumber>100000){
     			seqTs.SetSeq(rand() % 100000);
     		  ...
     		  if(std::find(seen_packets.begin(),seen_packets.end(),currentSequenceNumber)==seen_packets.end())
				{
     			 ...
     			 seen_packets.push_back(currentSequenceNumber);
     			 std::cout<<" IN Nist LTE UE NET Device Sending packet "<<currentSequenceNumber<<std::endl;
     		    }
