...
public:
  ...
  protected:
     ...
     */
    void AddDiscoveryApps (std::list<std::string> apps, bool rxtx);
    ...
    * \param rxtx 0 to monitor and 1 to announce
    */
    void RemoveDiscoveryApps (std::list<std::string> apps, bool rxtx);
  ...
  private:
     ...
     */
    std::list<std::string> m_monitorApps;

    /**
     * list of IDs of applications to announce
     */
    std::list<std::string> m_announceApps;
...
public:
  ...
  typedef void (* DiscoveryMonitoringTracedCallback)
    (const uint64_t imsi, const uint16_t cellId, const uint16_t rnti, const std::string proSeAppCode);
...
private:
  ...
  // discovery
  void DoAddDiscoveryApps (std::list<std::string> apps, bool rxtx);
  void DoRemoveDiscoveryApps (std::list<std::string> apps, bool rxtx);
   ...
   */
  TracedCallback<uint64_t, uint16_t, uint16_t, std::string> m_discoveryMonitoringTrace;
