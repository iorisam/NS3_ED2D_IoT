...
NS_LOG_COMPONENT_DEFINE ("NistLteAmc");

NS_OBJECT_ENSURE_REGISTERED (NistLteAmc);
...
TypeId
NistLteAmc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteAmc")
  .SetParent<Object> ()
  .AddConstructor<NistLteAmc> ()
  .AddAttribute ("Ber",
                 "The requested BER in assigning MCS (default is 0.00005).",
                 DoubleValue (0.00005),
                 MakeDoubleAccessor (&NistLteAmc::m_ber),
                 MakeDoubleChecker<double> ())
  .AddAttribute ("AmcModel",
                "AMC model used to assign CQI",
                 EnumValue (NistLteAmc::MiErrorModel),
                 MakeEnumAccessor (&NistLteAmc::m_amcModel),
                 MakeEnumChecker (NistLteAmc::MiErrorModel, "Vienna",
                                  NistLteAmc::PiroEW2010, "PiroEW2010",
                                  NistLteAmc::NistErrorModel, "Nist"));
  return tid;
}


int
NistLteAmc::GetCqiFromSpectralEfficiency (double s)
{
  NS_LOG_FUNCTION (s);
...
int
NistLteAmc::GetMcsFromCqi (int cqi)
{
  NS_LOG_FUNCTION (cqi);
...
int
NistLteAmc::GetTbSizeFromMcs (int mcs, int nprb)
{
  NS_LOG_FUNCTION (mcs);
...
double
NistLteAmc::GetSpectralEfficiencyFromCqi (int cqi)
{
  NS_LOG_FUNCTION (cqi);
  NS_ASSERT_MSG (cqi >= 0 && cqi <= 15, "CQI must be in [0..15] = " << cqi);
...
std::vector<int>
NistLteAmc::CreateCqiFeedbacks (const SpectrumValue& sinr, uint8_t rbgSize)
{
  ...
  /***** Functions for uplink/sidelink *****/

/**
 * Table of MCS index (IMCS) and its TBS index (ITBS). Taken from 3GPP TS
 * 36.213 v8.8.0 Table 7.1.7.1-1: _Modulation and TBS index table for PUSCH_.
 * The index of the vector (range 0-28) identifies the MCS index.
 */
...
int
NistLteAmc::GetUlTbSizeFromMcs (int mcs, int nprb)
{
  NS_LOG_FUNCTION (mcs);
