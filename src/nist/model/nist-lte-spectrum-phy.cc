...
void NistLteSpectrumPhy::DoDispose ()
{
  ...
  m_ltePhyRxDataEndErrorCallback2 = MakeNullCallback< void > ();
...
{
  NS_LOG_FUNCTION (this);
  std::cout<<"here is a collision thank you :::!!!!!!!!!!!!!!!!"<<std::endl;
  m_ltePhyRxDataEndErrorCallback2 = c;
}
...
void NistLteSpectrumPhy::SetNistLteRSSICallback(NistLteRSSICallback c)
{
	  NS_LOG_FUNCTION (this);
	  m_lteRssiCallback = c;
}
  ...
  switch (m_state)
  {
    ...
    case IDLE:
    {
      ...
      */
      NS_ASSERT (m_txPsd);
      m_txPacketBurst = pb;
      Ptr<PacketBurst> nonRedendant_TxPacketBurst = Create<PacketBurst>();
     // uint16_t numberOfPacketsToSend = 0;
           ...
           for (std::list<Ptr<Packet> >::const_iterator iter = pb->Begin (); iter
                != pb->End (); ++iter)
             {
     		 ...
     		 // packet->PeekHeader (iph);
               //
               if(std::find(seen_packets.begin(),seen_packets.end(),currentSequenceNumber)==seen_packets.end())
					{


            	   numberOfPacketsToSend++;
            	   std::cout<<this << "IN PHY NODE:" << GetDevice()->GetNode()->GetId() << " with seen packets size: "<< seen_packets.size()<<" is Sending packet "
            			   << currentSequenceNumber<<" from address : "<< iph.GetSource()<<" with control messages size "<< " to ip destination "<<iph.GetDestination()
            			   <<ctrlMsgList.size()<<std::endl;
            	   break;
					}else {
						std::cout<<this << "IN PHY NODE:" << GetDevice()->GetNode()->GetId() << " Sending packet already exists"<<std::endl;
						//m_txPacketBurst->RemovePacket((*iter));
					}
             }
...
*/
      ...
      //std::cout<<" node "<<GetDevice()->GetNode()->GetId()<<" is sending a tx burst "<<std::endl;
      ...
      txParams->packetBurst = m_txPacketBurst;
...
NistLteSpectrumPhy::StartRxData (Ptr<NistLteSpectrumSignalParametersDataFrame> params)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " ID:" << GetDevice()->GetNode()->GetId() << " state: " << m_state);
  std::cout<<" here we are in startRXDATA check it out"<<std::endl;
  switch (m_state)
    {
      case TX:
                    ...
                    if( (*ctrlIt)->GetMessageType () == NistLteControlMessage::MIB_SL)
                      {
                        ...
                        std::cout<<" they are trying to do something here "<<std::endl;
            ...
            if (params->slssId == m_slssId && (params->groupId == 0 || m_l1GroupIds.find (params->groupId) != m_l1GroupIds.end()))
              {
            	 if (!m_ltePhyRxSlssCallback.IsNull ())
            	                          {
            	                            ...
            	                            CalculateReceivedSignalStrength(params->slssId,params);
                ...
                NistSlRxPacketInfo_t packetInfo;
                packetInfo.m_rxPacketBurst = params->packetBurst;
                Ptr<PacketBurst> myPacketBurst = Create<PacketBurst> ();;

               /* if(packetInfo.m_rxPacketBurst != 0)
					 for (std::list<Ptr<Packet> >::const_iterator iter = packetInfo.m_rxPacketBurst->Begin (); iter
						  != packetInfo.m_rxPacketBurst->End (); ++iter)
					   {
						 Ptr<Packet> packet = (*iter)->Copy ();
						 //m_ltePhyTxEndCallback (packet);
						  SeqTsHeader seqTs;
					   ...
					   if(std::find(seen_packets.begin(),seen_packets.end(),currentSequenceNumber)==seen_packets.end())
					  		{
					      	  seen_packets.push_back(currentSequenceNumber);
					      	  seen_packets.push_back(currentSequenceNumber);

						 std::cout<<this << "IN PHY NODE:" << GetDevice()->GetNode()->GetId() << " with ip source "<< ipHeader.GetSource()
										<< " with ip destination "<< ipHeader.GetDestination()

										 << " is receiving packet " << currentSequenceNumber<<std::endl;
					  		}else{
					  			std::cout<<this << "IN PHY NODE:" << GetDevice()->GetNode()->GetId() << "  packet already exists"<<std::endl;
					  			//packetInfo.m_rxPacketBurst->RemovePacket((*iter)->Copy ());
					  		}
					}*/
                //packetInfo.m_rxPacketBurst = myPacketBurst;
                if (params->ctrlMsgList.size () >0)
                  {
...
NistLteSpectrumPhy::AddExpectedTb (uint16_t  rnti, uint8_t l1dst, uint8_t ndi, uint16_t size, uint8_t mcs, std::vector<int> map, uint8_t rv)
{
  ...
  tbId.m_rnti = rnti;
  tbId.m_l1dst = l1dst;
  NistexpectedSlTbs_t::iterator it;
...
NistLteSpectrumPhy::AddExpectedTb (uint16_t  rnti, uint8_t resPsdch, uint8_t ndi, std::vector<int> map, uint8_t rv)
{
  ...
  NistexpectedDiscTbs_t::iterator it;
...
{                               
  ...
  NistexpectedTbs_t::iterator itTb = m_expectedTbs.begin ();
  ...
  while (itTb!=m_expectedTbs.end ())
    {
      if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size ()>0)) // avoid to check for errors when there is no actual data transmitted
        {
          // retrieve HARQ info
    	  Nist_HarqProcessInfoList_t harqInfoList;
...
NistLteSpectrumPhy::EndRxSlData ()
{
      ...
      if (m_rxPacketInfo[i].m_rxPacketBurst) //if data packet
        {
          ...
          //NS_LOG_INFO("this node recieved packet from "<<tbId.m_rnti<<" received signal strength is "<< m_expectedSlTbs.at(tbId).sinr);
        }
    }
  ...
  if (m_dropRbOnCollisionEnabled)
    {
      ...
      for (NistexpectedSlTbs_t::iterator itTb = m_expectedSlTbs.begin (); itTb != m_expectedSlTbs.end (); itTb++ )
        {
  ...
  NistexpectedSlTbs_t::iterator itTb = m_expectedSlTbs.begin ();
  ...
  if (ctrlMessageFound)
    ...
    if (m_errorModelHarqD2dDiscoveryEnabled)
    {
      ...
      //           
      NistexpectedDiscTbs_t::iterator itTbDisc = m_expectedDiscTbs.begin ();
          ...
          if (!ctrlError)
            {
              ...
              /*NistSlDiscMsg disc = msg->GetSlDiscMessage ();
              NS_LOG_INFO (" Expecting An overflow error ");
              NS_LOG_DEBUG (this << " from RNTI " << disc.m_rnti << " ProSeAppCode " << disc.m_proSeAppCode.to_ulong () << " SINR " << (*it).sinr);*/
            } 
        }
...
double NistLteSpectrumPhy::CalculateReceivedSignalStrength(uint64_t slssid, Ptr<NistLteSpectrumSignalParametersSlFrame> params){

	 double sum = 0.0;
	      uint16_t nRB = 0;
	      Values::const_iterator itPi;
	      for (itPi = params->psd->ConstValuesBegin(); itPi != params->psd->ConstValuesEnd(); itPi++)
	        {
	          if((*itPi))
	            {
	      ...
	      NS_LOG_INFO("*-*-*-*-*-* this is node "<<GetDevice()->GetNode()->GetId()<<" received from "<<params->nodeId<<" signal strength "<<s_rsrp_dBm);
	      if (!m_lteRssiCallback.IsNull ())
	      m_lteRssiCallback(params->nodeId,s_rsrp_dBm);
	      return s_rsrp_dBm;
}
...
void 
NistLteSpectrumPhy::AddDiscTxApps (std::list<std::string> apps)
{
    m_discTxApps = apps;
}

void 
NistLteSpectrumPhy::AddDiscRxApps (std::list<std::string> apps)
{
  m_discRxApps = apps;
}
...
NistLteSpectrumPhy::FilterRxApps (NistSlDiscMsg disc)
{
  ...
  bool exist = true;
 /* for (std::list<std::string>::iterator it = m_discRxApps.begin (); it != m_discRxApps.end (); ++it)
  {
