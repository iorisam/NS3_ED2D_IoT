...
NistLteHarqPhy::NistLteHarqPhy ()
{
  ...
  std::vector <Nist_HarqProcessInfoList_t> dlHarqLayer0;
  dlHarqLayer0.resize (8);
  std::vector <Nist_HarqProcessInfoList_t> dlHarqLayer1;
...
NistLteHarqPhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  std::map <uint16_t, std::vector <Nist_HarqProcessInfoList_t> >:: iterator it;
  for (it = m_miUlHarqProcessesInfoMap.begin (); it != m_miUlHarqProcessesInfoMap.end (); it++)
    {
      (*it).second.erase ((*it).second.begin ());
      Nist_HarqProcessInfoList_t h;
...
NistLteHarqPhy::GetAccumulatedMiDl (uint8_t harqProcId, uint8_t layer)
{
  ...
  Nist_HarqProcessInfoList_t list = m_miDlHarqProcessesInfoMap.at (layer).at (harqProcId);
  double mi = 0.0;
...
NistLteHarqPhy::GetAccumulatedMiUl (uint16_t rnti)
{
  ...
  Nist_HarqProcessInfoList_t list = (*it).second.at (0);
  double mi = 0.0;
  for (uint8_t i = 0; i < list.size (); i++)
    {
      mi += list.at (i).m_mi;
...
NistLteHarqPhy::GetHarqProcessInfoUl (uint16_t rnti, uint8_t harqProcId)
{
  ...
  if (it==m_miUlHarqProcessesInfoMap.end ())
    {
      // new entry
      std::vector <Nist_HarqProcessInfoList_t> harqList;
      harqList.resize (8);
      m_miUlHarqProcessesInfoMap.insert (std::pair <uint16_t, std::vector <Nist_HarqProcessInfoList_t> > (rnti, harqList));
...
NistLteHarqPhy::GetHarqProcessInfoSl (uint16_t rnti, uint8_t l1dst)
{
  ...
  if (it==m_miSlHarqProcessesInfoMap.end ())
    {
      // new entry
	  Nist_HarqProcessInfoList_t harqInfo;
      std::map <uint8_t, Nist_HarqProcessInfoList_t> map;
      map.insert (std::pair <uint8_t, Nist_HarqProcessInfoList_t> (l1dst, harqInfo));
      m_miSlHarqProcessesInfoMap.insert (std::pair <uint16_t, std::map <uint8_t, Nist_HarqProcessInfoList_t> > (rnti, map));
      return (harqInfo);
    }
  else
    {
      ...
      if (it2==it->second.end ())
        {
    	  Nist_HarqProcessInfoList_t harqInfo;
          std::map <uint8_t, Nist_HarqProcessInfoList_t> map;
          it->second.insert (std::pair <uint8_t, Nist_HarqProcessInfoList_t> (l1dst, harqInfo));
          return (harqInfo);
        }
      else
        {
  ...
  if (it==m_miDiscHarqProcessesInfoMap.end ())
    {
	  ...
	  Nist_HarqProcessInfoList_t harqInfo;
      std::map <uint8_t, Nist_HarqProcessInfoList_t> map;
      map.insert (std::pair <uint8_t, Nist_HarqProcessInfoList_t> (resPsdch, harqInfo));
      m_miDiscHarqProcessesInfoMap.insert (std::pair <uint16_t, std::map <uint8_t, Nist_HarqProcessInfoList_t> > (rnti, map));
      return (harqInfo);
    }
  else
    {
      ...
      if (it2==it->second.end ())
        {
    	  Nist_HarqProcessInfoList_t harqInfo;
          std::map <uint8_t, Nist_HarqProcessInfoList_t> map;
          it->second.insert (std::pair <uint8_t, Nist_HarqProcessInfoList_t> (resPsdch, harqInfo));
          return (harqInfo);
        }
      else
        {
