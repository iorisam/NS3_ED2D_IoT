...
NS_LOG_COMPONENT_DEFINE ("NistLteSlInterference");

NistLteSlInterference::NistLteSlInterference ()
  : m_receiving (false),
    m_lastSignalId (0),
...
void 
NistLteSlInterference::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
TypeId
NistLteSlInterference::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteSlInterference")
    .SetParent<Object> ()
  ;
  return tid;
}
...
void
NistLteSlInterference::StartRx (Ptr<const SpectrumValue> rxPsd)
{ 
  ...
  if (m_receiving == false) {
   ...
   // std::cout<<"i am having a collision here and now "<<std::endl;
  }
  ...
  for (std::list<Ptr<NistLteSlChunkProcessor> >::const_iterator it = m_rsPowerChunkProcessorList.begin (); it != m_rsPowerChunkProcessorList.end (); ++it)
    {
      (*it)->Start (init);
    }
  for (std::list<Ptr<NistLteSlChunkProcessor> >::const_iterator it = m_interfChunkProcessorList.begin (); it != m_interfChunkProcessorList.end (); ++it)
    {
      (*it)->Start (init);
    }
  for (std::list<Ptr<NistLteSlChunkProcessor> >::const_iterator it = m_sinrChunkProcessorList.begin (); it != m_sinrChunkProcessorList.end (); ++it)
    {
      (*it)->Start (init); 
    }
}
...
void
NistLteSlInterference::AddSignal (Ptr<const SpectrumValue> spd, const Time duration)
{
  ...
  Simulator::Schedule (duration, &NistLteSlInterference::DoSubtractSignal, this, spd, signalId);
}


void
NistLteSlInterference::DoAddSignal  (Ptr<const SpectrumValue> spd)
{ 
...
void
NistLteSlInterference::DoSubtractSignal  (Ptr<const SpectrumValue> spd, uint32_t signalId)
{ 
  NS_LOG_FUNCTION (this << *spd);
  ConditionallyEvaluateChunk ();   
...
void
NistLteSlInterference::ConditionallyEvaluateChunk ()
{
  NS_LOG_FUNCTION (this);
  if (m_receiving)
    {
...
void
NistLteSlInterference::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
void
NistLteSlInterference::AddRsPowerChunkProcessor (Ptr<NistLteSlChunkProcessor> p)
{
...
void
NistLteSlInterference::AddSinrChunkProcessor (Ptr<NistLteSlChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
...
void
NistLteSlInterference::AddInterferenceChunkProcessor (Ptr<NistLteSlChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
