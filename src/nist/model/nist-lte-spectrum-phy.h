...
typedef std::map<NistTbId_t, NisttbInfo_t> NistexpectedTbs_t;
...
typedef std::map<NistSlTbId_t, NistSltbInfo_t> NistexpectedSlTbs_t;
...
typedef std::map<NistDiscTbId_t, NistDisctbInfo_t> NistexpectedDiscTbs_t;
...
/**
* This method is created by sam to get the rssi of a received packet from side link
*  to notify the APP
*
* @param rnti the id of sender
* @param rssi the recieved signal strenght
*/
typedef Callback< void, uint16_t, double > NistLteRSSICallback;
...
public:
  ...
  */
  ...
  /**
    * set the callback for the rssi, as part of the
    * interconnections between the PHY and the APP
    *
    * @param c the callback
    */
  void SetNistLteRSSICallback (NistLteRSSICallback c);
  ...
  */
  ...
  void AddDiscTxApps (std::list<std::string> apps);
  void AddDiscRxApps (std::list<std::string> apps);
     
  void SetDiscNumRetx (uint8_t retx);

  std::vector<int> seen_packets;
  std::vector<int> second_seen_packets;
...
private:
  ...
  double CalculateReceivedSignalStrength(uint64_t slssid, Ptr<NistLteSpectrumSignalParametersSlFrame> params);
  ...
  NistLtePhyRxDataEndErrorCallback   m_ltePhyRxDataEndErrorCallback2;
  NistLtePhyRxDataEndOkCallback      m_ltePhyRxDataEndOkCallback;
  
  NistLteRSSICallback m_lteRssiCallback;
  ...
  NistexpectedTbs_t m_expectedTbs;
  NistexpectedDiscTbs_t m_expectedDiscTbs;
  ...
  NistexpectedSlTbs_t m_expectedSlTbs;
  ...
  std::list<std::string> m_discTxApps;
  std::list<std::string> m_discRxApps;
