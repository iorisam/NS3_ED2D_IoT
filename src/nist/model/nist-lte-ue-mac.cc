...
public:
  ...
  virtual void ModifyDiscTxApps (std::list<std::string> apps);
  virtual void ModifyDiscRxApps (std::list<std::string> apps);
...
void
NistUeMemberLteUeCmacSapProvider::ModifyDiscTxApps (std::list<std::string> apps)
{
  m_mac->DoModifyDiscTxApps (apps);
}

void
NistUeMemberLteUeCmacSapProvider::ModifyDiscRxApps (std::list<std::string> apps)
{
...
void 
NistLteUeMac::DoModifyDiscTxApps (std::list<std::string> apps)
{
  m_discTxApps = apps;
...
void 
NistLteUeMac::DoModifyDiscRxApps (std::list<std::string> apps)
{
  m_discRxApps = apps;
...
NistLteUeMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  if (allocIt != m_discTxPools.m_psdchTx.end() && (*allocIt).subframe.frameNo == frameNo && (*allocIt).subframe.subframeNo == subframeNo)
  {
    ...
    for (std::list<std::string>::iterator txApp = m_discTxApps.begin (); txApp != m_discTxApps.end (); ++txApp)
    {
