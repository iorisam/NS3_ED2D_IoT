    ...
    case OFF:
      {
	   ...
	   uint32_t packet_identification = ipv4Header.GetIdentification();
        std::cout<<" Node "<< m_imsi <<" TX  this is the nist epc ue nas packet sequence number is "<<packet_identification
        		<<" packet source is  "<<ipv4Header.GetSource()<<" and destination is "<<ipv4Header.GetDestination()<<std::endl;

        std::map<Address, seenPacketsIds>::iterator it;
        it = seen_packets.find(ipv4Header.GetSource());

        if(it != seen_packets.end())
           if(std::find(it->second.ids.begin(),it->second.ids.end(),packet_identification)==it->second.ids.end())
       				{
        	   it->second.ids.push_back(packet_identification);
        	   it->second.ids.push_back(packet_identification);
            		    }else {
            		    	NS_LOG_WARN (this << " Duplicated Packet, discarding packet");
            		    	      return false;
            		    	      break;
            		    }
        else {
        	std::vector<uint32_t> temp;
        	temp.push_back(packet_identification);
        	seenPacketsIds freshCache;
        	freshCache.ids = temp;

        	std::pair<Address,seenPacketsIds> m_pair(ipv4Header.GetSource(),freshCache);
        	seen_packets.insert(m_pair);
        }
        for (std::list<Ptr<NistSlTft> >::iterator it = m_slBearersActivatedList.begin ();
             it != m_slBearersActivatedList.end ();
             it++)
...
NistEpcUeNas::DoRecvData (Ptr<Packet> packet)
{
	...
	uint32_t currentSequenceNumber = seqTs.GetSeq ();
	 ...
	 std::cout<<" Node "<< m_imsi <<" RX this is the nist epc ue nas packet sequence number is "<<ipv4Header.GetIdentification()
	        		<<" packet source is  "<<ipv4Header.GetSource()<<" and destination is "<<ipv4Header.GetDestination()<<std::endl;
  NS_LOG_FUNCTION (this << packet);
  NS_LOG_INFO( "this is node is receiving packet with size " <<packet->GetSize());
  //BreakpointFallback();
...
void 
NistEpcUeNas::AddDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
  m_asSapProvider->AddDiscoveryApps (apps, rxtx);
}
void 
NistEpcUeNas::RemoveDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
