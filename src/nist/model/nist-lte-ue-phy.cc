...
Ptr<NistLteSpectrumPhy>
NistLteUePhy::GetSlSpectrumPhy () const
{
...
NistLteUePhy::PhyPduReceived (Ptr<Packet> p)
{
	...
	/*Ptr<Packet> p2 = p->Copy();
	  SeqTsHeader seqTs;
	  p2->RemoveHeader (seqTs);
	   uint32_t currentSequenceNumber = seqTs.GetSeq ();
	   std::cout<<" receiving packet at phy level with seq "<<currentSequenceNumber<<std::endl;*/
...
Ptr<SpectrumValue>
NistLteUePhy::CreateTxPowerSpectralDensity ()
{
  NS_LOG_FUNCTION (this);
...
//             NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
      ...
      else if (msg->GetMessageType () == NistLteControlMessage::UL_DCI)
        {
          ...
          Nist_HarqProcessInfoList_t harqInfoList = m_harqPhyModule->GetHarqProcessInfoUl (m_rnti, 0);
      ...
      if (rbMask.size () == 0)
        {
          ...
          for (ctrlIt=ctrlMsg.begin() ; ctrlIt != ctrlMsg.end() && !sciDiscfound; ctrlIt++) {
              sciDiscfound = (*ctrlIt)->GetMessageType () == NistLteControlMessage::SCI || (*ctrlIt)->GetMessageType () == NistLteControlMessage::SL_DISC_MSG;
              ...
              while (ctrlIt != ctrlMsg.end())
                {
                  ...
                  if ((*ctrlIt)->GetMessageType () == NistLteControlMessage::DL_CQI
                      || (*ctrlIt)->GetMessageType () == NistLteControlMessage::BSR)
                    {
                  ...
                  if (!mibSLfound)
                    {
                      if(m_ueSlssScanningInProgress)
                        ...
                        {
                    	  std::cout<<" inside nistlteudphy trying to call startTxSlDataFrame 1"<<std::endl;
                          SetSubChannelsForTransmission (rbMask);
                          m_uplinkSpectrumPhy->StartTxSlDataFrame (pb, ctrlMsg, UL_DATA_DURATION, m_slTxPoolInfo.m_currentGrants.begin()->second.m_grant.m_groupDstId);
                        }
                    }
                  ...
                  if (ctrlMsg.size ()>0 && sciDiscfound)
                        ...
                        if (!mibSLfound)
                          {
                            if(m_ueSlssScanningInProgress)
                              ...
                              {
                            	std::cout<<" inside nistlteudphy trying to call startTxSlDataFrame 2"<<std::endl;
                                SetSubChannelsForTransmission (slRb);
                                m_uplinkSpectrumPhy->StartTxSlDataFrame (pb, ctrlMsg, UL_DATA_DURATION, sci.m_groupDstId);
                              }
                          }
                    ...
                    else if ((*msgIt)->GetMessageType () == NistLteControlMessage::SL_DISC_MSG)
                    {
                      ...
                      if (grantIt == m_discTxPools.m_currentGrants.end ()) 
                      {
                        ...
                        //
                        std::cout<<" inside nistlteudphy trying to call startTxSlDataFrame 3"<<std::endl;
                        ...
                        for (std::list<Ptr<NistLteControlMessage> >::iterator msg = ctrlMsg.begin(); msg != ctrlMsg.end(); ++msg)
                        {
                          ...
                          {
                            NistSlDiscMsg disc = msg2->GetSlDiscMessage ();
                            m_discoveryAnnouncementTrace (m_cellId, m_rnti,disc.m_proSeAppCode.to_string());
                          }
                        }
          ...
          if (mibSLfound)
            {
              ...
              if(m_ueSlssScanningInProgress)
              ...
              else
                {
                  ...
                  SetSubChannelsForTransmission(dlRb);
                  std::cout<<" inside nistlteudphy trying to call startTxSlDataFrame 4"<<std::endl;
                  m_uplinkSpectrumPhy->StartTxSlDataFrame(pb, ctrlMsg, UL_DATA_DURATION,m_slTxPoolInfo.m_currentGrants.begin()->second.m_grant.m_groupDstId);
                }
            }
        }
...
void 
NistLteUePhy::DoAddDiscTxApps (std::list<std::string> apps)
{
  m_discTxApps = apps;
...
void 
NistLteUePhy::DoAddDiscRxApps (std::list<std::string> apps)
{
  m_discRxApps = apps;
...
NistLteUePhy::SwitchToState (State newState)
{
  ...
  std::cout<<"i am out of this function "<<std::endl;
}
  ...
  if(true)
    {
      ...
      //std::cout<<"*-*-*-*-*-* this is node "<<m_rnti<<" received from "<<slssid<<" signal strength "<<s_rsrp_dBm<<std::endl;
