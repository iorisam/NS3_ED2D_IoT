...
void 
LteUeRrcSl::AddDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
  for (std::list<std::string>::iterator it = apps.begin (); it != apps.end (); ++it)
  {
    if (rxtx)
    {
...
void 
LteUeRrcSl::RemoveDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
  for (std::list<std::string>::iterator it = apps.begin (); it != apps.end (); ++it)
  {
    if (rxtx)
...
void
NistLteUeRrc::DoAddDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this);
...
void
NistLteUeRrc::DoRemoveDiscoveryApps (std::list<std::string> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this);
...
void NistLteUeRrc::DoNotifyDiscoveryReception (Ptr<NistLteControlMessage> msg)
{
  ...
  NS_LOG_INFO ("discovery message received by " << m_rnti << ", proSeAppCode = " << disc.m_proSeAppCode.to_string());
       m_discoveryMonitoringTrace (m_imsi, m_cellId, disc.m_rnti, disc.m_proSeAppCode.to_string());

  /*for (std::list<std::string>::iterator it = m_sidelinkConfiguration->m_monitorApps.begin (); it != m_sidelinkConfiguration->m_monitorApps.end (); ++it)
  {
    if (true)
    {
